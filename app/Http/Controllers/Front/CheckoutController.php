...
class CheckoutController extends Controller
{
    ...
    public function confirmOrder(Request $request)
    {
        ...
        if ($user->firstname !== $request->name || $user->address !== $request->address) {
        ...
        if ($user) {
            ...
            $order->cart_json = json_encode($cart->content());
            ...
            return redirect('/dashboard/orders')->with('s_message', "Order Successfully!");
        } else {
            dd('User Not Found');
            $user = new User();
            $user->mobile = $request->mobile;
            ...
            return response()->json(['otpRequired' => true]); // Indicate OTP form should be shown
        }
    }
    ...
    public function placeOrder(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if ($request->payment_method_id == 6) {
                ...
                $tran_id = 'kzinternational-' . rand(10000, 20000);
                ...
                $fields = array(
                    ...
                    'signature_key' => 'fc7fb156f98b03bd3046da296c21b730'
                );

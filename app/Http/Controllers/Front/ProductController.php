...
class ProductController extends Controller
{
            ...
            if (count($get_products) > 0) {
                ...
                foreach ($get_products->take(1) as $first_product) {
                    $output .= '<div class="banner-img">
                                                <a title="' . $first_product->title . '" href="' . url('product/' . $first_product->slug) . '"><img
    ...
    function product_search_data(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if (!empty($product_data)) {
                ...
                if (!empty($category_filter[0])) {
                    $cat_info = DB::table('categories')->where('id', $category_filter[0])->first();
                    // <img src="'.SM::sm_get_the_src($cat_info->image, 1017, 336) .'" alt="'. $cat_info->title .'" class="img-responsive">
                    $output .= '
                                <div class="category-slider">
                                <h2 class="page-heading">
                                <span class="">' . $cat_info->title . ' </span>
                                </h2>
                                <ul class="display-product-option" style="width: 63px;margin-top: 0!important;">
                ...
                foreach ($product_data as $product) {
                    if ($product->product_type == 2) {
                        ...
                        $att_data = SM::getAttributeByProductId($product->id);
                        $attribute_image = !empty($att_data->attribute_image) ? $att_data->attribute_image : $product->image;
                        ...
                        $output .= '
                            <li>
                                <div class="product-container">
                                    <div class="left-block">
                                        <a href="' . url('product/' . $product->slug) . '">
                                            <img class="img-responsive" alt="' . htmlspecialchars($product->title) . '" src="' . SM::sm_get_the_src($attribute_image, 268, 268) . '" />
                                        </a>';

                        if ($product->sale_price > 0) {
                            $output .= '
                                <div class="price-percent-reduction2">' . SM::productDiscount($product->id) . '% OFF</div>';
                        }
                        ...
                        $output .= '
                                    ...
                                    <div class="right-block">
                                        <h5 class="product-name">
                                            <a href="' . url('product/' . $product->slug) . '">' . htmlspecialchars($product->title) . '</a>
                                        </h5>
                                        <div class="content_price">';
                        ...
                        $output .= '
                                        ...
                                        <div class="info-orther">
                                            ...
                                            <p class="availability">Availability: <span>' . htmlspecialchars($product->stock_status) . '</span></p>
                                            <div class="product-desc">' . htmlspecialchars($product->short_description) . '</div>
                                        </div>';

                        // Add color options
                        $colors = SM::productAttributeColor($product->id);
                        if (!empty($colors)) {
                            $output .= '<div class="product-colors">';
                            foreach ($colors as $color) {
                                $output .= '
                                    <label style="background-color: ' . htmlspecialchars($color->color_code) . ';" class="color_btn_looks">
                                        <input type="radio" name="product_attribute_color" class="color-btn color-attr" data-color-id="' . $color->color_id . '" style="display: none;" value="' . $color->color_id . '" />
                                    </label>';
                            }
                            $output .= '</div>';
                        }
                        ...
                        if (!empty($sizes)) {
                            $output .= '<div class="product-sizes">';
                            foreach ($sizes as $size) {
                                $output .= '
                                    <label class="size_btn_looks">
                                        <input type="radio" name="size" class="size-btn size-attr" data-size-id="' . $size->id . '" style="display: none;" value="' . $size->id . '" />
                                        ' . htmlspecialchars($size->title) . '
                                    </label>';
                            }
                            $output .= '</div>';
                        }
                        ...
                        $output .= '
                                        <div class="quick-view">' . SM::quickViewHtml($product->id, $product->slug) . SM::addToCartButton($product->id, $product->regular_price, $product->sale_price) . '</div>
                                    </div>
                                </div>
                            </li>';
                    } else {
                        $output .= ' 
                          <li>
                            <div class="product-container">
                                <div class="left-block">
                                    <a href="' . url('product/' . $product->slug) . '">
                                        <img class="img-responsive" alt="product" src="' . SM::sm_get_the_src($product->image, 268, 268) . '" />
                                    </a>
                        ...
                        $output .= ' 
                                    ...
                                    <div class="quick-view">';
                        $output .=  SM::addToCartButton($product->id, $product->regular_price, $product->sale_price) . SM::quickViewHtml($product->id, $product->slug) . '
                                    </div>
                                </div>
        ...
        if ($request->ajax()) {
            ...
            if ($attribute_product) {
                ...
                if (!empty($image)) {
                    ...
                    $data['attribute_image'] = '<img id="product-zoom" src="' . url('/storage/uploads/' . $final_imagename . '_500x500.' . $final_extension) . '" data-zoom-image="' . url('/storage/uploads/' . $final_imagename . '_1000x1000.' . $final_extension) . '" class="image-style" alt="Product-11">';
                } else {
                    $final_imagename = '';
                    $final_extension = '';
    ...
    public function getProductByColor(Request $request)
    {
        $product_id = $request->product_id;
        $color_id = $request->color_id;
        // $filtration_product = AttributeProduct::where('product_id', $product_id)
        //     ->where('color_id', $color_id)
        //     ->get();

        $filtration_product = DB::table('attribute_product')
            ->join('attributes', 'attribute_product.attribute_id', 'attributes.id')
            ->where('product_id', $product_id)
            ->where('color_id', $color_id)
            ->groupBy('attribute_id')
        ...
        return Response()->json($filtration_product);
    }
                ...
                if ($product->product_type == 2) {
                    ...
                    $output .= '<li class="col-sx-12 col-sm-4 col-md-3">
                                <div class="product-container">
                                    <div class="left-block">
                                        <a href="' . url('product/' . $product->slug) . '">
                                            <img class="img-responsive" alt="product" src="' . SM::sm_get_the_src($attribute_image, 268, 268) . '" />
                                        </a>
                                    </div>
                                    <div class="right-block">
                                    <h5 class="product-name">
                    ...
                    ' . $product->short_description . '
                                ...
                                <div class="quick-view">';
                    $output .= SM::quickViewHtml($product->id, $product->slug) . SM::addToCartButton($product->id, $product->regular_price, $product->sale_price) . '  
                                    </div>
                            </div>
                        </li>
                    ';
